...
def test(test_loader, model_dict, criterion_dict, visualizer, epoch, args, fixed_z, fixed_rand_z, iter=0):
    ...
    img_paths = []
    with torch.no_grad():
        for i, (img, label, landmarks, img_path) in tqdm.tqdm(enumerate(test_loader)):
            img_paths.extend(img_path)
            if args.lambda_dis > 0:
                # concated_img = torch.cat((img, fixed_z.cpu().view(args.test_size, -1, 1, 1).repeat(1, 1, img.size(2), img.size(3))),
                #                          1)
                fake = model_dict['G'](img, fixed_z.cpu())
                rand_fake = model_dict['G'](img, fixed_rand_z)
    ...
    import pickle
    with open('img_paths.pickle', 'wb') as f:
